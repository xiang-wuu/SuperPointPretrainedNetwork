...
class SuperPointFrontend(object):
    ...
    def __init__(self, weights_path, nms_dist, conf_thresh, nn_thresh,
        ...
        if ".pth" in weights_path:
            ...
            self.net.eval()
        elif ".engine" in weights_path:
            print("infer using tensorrt")
            cuda.init()
            self.device = cuda.Device(0)  # enter your Gpu id here
            self.ctx = self.device.make_context()
            self.engine, self.context = build_engine(weights_path)
    ...
    def run(self, img):
        ...
        semi, coarse_desc = outs[0], desc  # outs[1]
